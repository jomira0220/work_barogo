...
import {
  ...
  getImgSize,
  createSrcSet,
} from "@/components/utils/reactQuillControl";
...
export default function ReactQuillContainer({
  content,
  className,
  readOnly,
  ...props
  ...
  //<undefined>(): MutableRefObject<undefined>
  ...
  useEffect(() => {
    ...
    if (content) {
      ...
      const length = 0;
      const rangeStatic = { index, length };
      editor?.setSelection(rangeStatic);
    }
  }, [content, props.type]);
  ...
  const imageCompressHandler = useCallback(
    (file: File) => {
      ...
      if (targetType !== "image/gif") {
        ...
        imageCompression(targetFile, {
          maxSizeMB: 9,
          useWebWorker: true,
          fileType: targetType,
        })
          .then((res) => {
            const compressFile = new File([res], fileName, {
              type: targetType,
            });
  ...
  //! 드래그앤 드롭 이미지 업로드 처리
  const imageDragAndDrop = useCallback(
  ...
  const videoHandler = useCallback(() => {
    ...
    const getVideoUrl = (url: string) => {
      if (
        ...
        url === null ||
        url === undefined ||
        (url !== null && url.includes("https://") === false)
      ) {
    ...
    const urlPrompt = prompt("유투브 영상 링크를 입력해주세요: ") || "";
    const url = getVideoUrl(urlPrompt);

    if (url !== null && quillRef.current) {
      const editor = quillRef.current.getEditor();
  ...
  // ! Quill 내용 변경시
  const QuillChangeSet = async (quillRef: any) => {
    if (quillRef.current) {
  ...
  const modules = useMemo(() => {
    ...
    let readOnlyModules;
    if (readOnly) {
      const modulesArr = Object.entries(modules).filter(([key]) => {
        const removeKey = ["imageDropAndPaste", "imageResize"];
        return !removeKey.includes(key);
      });
      readOnlyModules = Object.fromEntries(modulesArr);
    }

    return readOnly ? readOnlyModules : modules;
  ...
  return (
    <QuillStyleBox type={props.type}>
      <ReactQuill
        className={
          (className ? className : "") + (readOnly ? " ql-readOnly" : "")
        }
        theme="snow"
        ref={quillRef}
...
const QuillStyleBox = styled.div<{ type: string }>`
