...
export function Filter({ filterCategory, filterTitleSet, filterTotalData, setFilterTotalData, setFilterChangeCheck }) {

  let { filterTitle, filterTitleEn, filterType, filterSetItem } = FilterSet(filterTitleSet)[0];
  ...
  const basicDate = (type) => {
    const startDate = query[filterDataName[0]] ? query[filterDataName[0]] : filterTotalData[filterDataName[0]];
    const endDate = query[filterDataName[1]] ? query[filterDataName[1]] : filterTotalData[filterDataName[1]];
    if (type === "start") {
  ...
  const [radioStartDate, setRadioStartDate] = useState(query["radioStartDate"] ? new Date(query["radioStartDate"]) : new Date())
  const [radioEndDate, setRadioEndDate] = useState(query["radioEndDate"] ? new Date(query["radioEndDate"]) : new Date())
  const radioDateChange = (date, type) => {
    type === "start" ? setRadioStartDate(date) : setRadioEndDate(date)
  }

  //! 기간별 회원 조건 항목 기간 선택 클릭시에만 날짜선택창 노출 처리용
  const [MemberPeriod, setMemberPeriod] = useState(query["radioStartDate"] ? true : false);
  ...
  // 상단 필터 체크 박스 선택 처리용
  const CheckEl = (e, title) => {
    let checkArr = [];
    const checkbox = document.querySelectorAll(`.${filterCategory} input[name=${filterTitleEn}]`);
    const allCheck = e.target.parentNode.parentNode.parentNode.querySelector(".allCheck");
        ...
        {filterType === "일반" ? (
              ...
              return (
                <li key={index} className={styles.filter_Item}>
                  <input
                    ...
                    defaultChecked={
                      query[filterTitleEn]
                        ? query[filterTitleEn].includes(item.value) ? true : false
                        : index === 0 ? true : false} // 첫번째 값은 기본 선택값으로 설정
                  />
                  <label htmlFor={`${filterCategory}_${filterTitleEn}_${index}`}>
                    {item.title}
                  </label>
                </li>
              )
          ...
          <>
            <div className={styles.filter_datepicker}>
              ...
              />
              <input type="hidden" name={`${filterDataName[0]}`} value={startDate} />
              {filterType === "날짜" && (
                <>
                  ...
                  />
                  <input type="hidden" name={`${filterDataName[1]}`} value={endDate} />
                </>
              )}
            </div>
          </>
          ...
          <div className={styles.filter_Number}>
            <SentInput className="numberSet" name={`${filterTitleEn}`} placeholder="0" defaultValue={query["likeCountUpper"] ? query.likeCountUpper : 0} />개 이상만 조회
          </div>
        ) : filterType === "라디오" && (
          <div className={styles.filter_select}>
            <ul className={styles.filter_list}>
              {filterSetItem.map((item, index) => {
                ...
                const monthCheck = query["countBaseDate"] === monthSet(new Date(), 6);
                const sevenCheck = query["countBaseDate"] === daySet(new Date(), 7);
                // console.log(query[filterTitleEn], filterTitleEn)
                const checkedSet = Object.keys(query).length === 0
                ...
                return (
                  <li key={index} className={styles.filter_Item + " " + (item.title === "기간선택" ? ` ${styles.radioDateSet}` : "")}>
                    <input
                      id={`${filterCategory}_${filterTitleEn}_${index}`}
                      type="radio"
                      name={`${filterTitleEn}`}
                    ...
                    {item.title === "기간선택" && MemberPeriod && (
                      ...
                      <div className={styles.filter_datepicker}>
                        ...
                        />
                        <input type="hidden" name={`기준날짜 시작`} value={radioStartDate} />
                        <span className={styles.data_icon}>~</span>
                        <DatePickerBox
                          maxDate={new Date()}
                        ...
                        />
                        <input type="hidden" name={`기준날짜 종료`} value={radioEndDate} />
                      </div>
                    )}
                  </li>
                )
