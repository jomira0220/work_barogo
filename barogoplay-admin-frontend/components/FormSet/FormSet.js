...
export function FormWarp({ children, ...props }) {
  return (<div className={styles.formWarp} {...props}>{children}</div>)
}

export function StyleBox({ children, styletype, title, ...props }) {
  return (
    <div className={styletype === "line"
      ? styles.lineBox
...
export function SentDate({ title, ...props }) {
  return (
    title ? (
      <div className={styles.dateBox} >
        <h3>{title}{props.info && <span className={styles.noticeSpan}>{props.info}</span>}</h3>
        <DatePickerBox {...props} />
      </div>
    ) : <DatePickerBox {...props} />
  )
...
export function SentTarget({ FileView, setFileView, limitCount, readOnly, ...props }) {
  ...
  const popClose = () => { setMemberListPopup(false); }
  ...
  let processedUserList, processedUserListCount, repeatedUserList, repeatedUserListCount, unExistUsersList, unExistUsersListCount
  if (FileView !== null) {
    // 발송될 회원리스트
    processedUserList = FileView.data.processedUserList
    processedUserListCount = Number(processedUserList.length).toLocaleString("ko-KR")
    // 중복된 회원리스트
    repeatedUserList = FileView.data.repeatedUsers.userList
    repeatedUserListCount = Number(repeatedUserList.length).toLocaleString("ko-KR")
    // 존재하지 않는(탈퇴) 회원리스트
    unExistUsersList = FileView.data.unExistUsers.userList
    unExistUsersListCount = Number(unExistUsersList.length).toLocaleString("ko-KR")
  }
  ...
  const UserListTable = ({ title, userList }) => {
    ...
    return (
      <div className={styles.toggleTableBox} onClick={(e) => ToggleTableBox(e)}>
        <h4>{title}({userListCount + "명"}){userList.length > 0 && <LineBasicArrow />}</h4>
  ...
  const MessageTargetBrand = (props) => {
    ...
    const messageTargetBrandValueList = { 'unMapped': "미연동", 'BAROGO': "바로고", 'DEALVER': "딜버", 'MOALINE': "모아라인" }
    return (
      Object.keys(messageTargetBrandValueList).map((item, index) => {
        return (
          <label htmlFor={item !== "unMapped" ? `brand_${item}` : item} key={index}>
            <input
    ...
    <>
      <div className={styles.radioBox}>
        {props.name === "badgeGrantTarget" ? (
          <SaveMemberListChoicePop setFileView={setFileView} popClose={popClose} type="box" />
        ) : (
          <>
            <h3>발송 대상</h3>
            <fieldset className={styles.targetBox}>
              <div style={{ flexDirection: "column", alignItems: "flex-start" }}>
                <label htmlFor="allMember">
                  ...
                  {props.name === "giftSentTarget" ? "전체 회원" : "전체 회원 중 선택"}
                </label>
                {/* 
                ...
                */}
                ...
                {props.name !== "giftSentTarget" && BrandSetShow && (
                  ...
                  <div className={styles.targetForm}>
                    <MessageTargetBrand readOnly={readOnly} FileView={FileView} />
                  </div>
                )}
              ...
              <div>
                <label htmlFor="saveListMember">
                  <input
                    ...
                    onChange={(e) => e.target.checked ? setBrandSetShow(false) : setBrandSetShow(true)} />
                  저장된 회원리스트 선택
            ...
            {/* 저장된 회원리스트 선택 업로드 팝업 */}
            {memberListPopup && (
              <SaveMemberListChoicePop setFileView={setFileView} popClose={popClose} type="popup" />
            )}
          </>
        )}

        {FileView &&
          (<div className={styles.fileView + (readOnly ? " " + styles.marginZero : "")}>
            {
              limitCount && FileView.data.length > limitCount && (
                // 선물하기 관련임
                <p className={styles.limitCount}>
                  발송 대상 : {processedUserListCount}명  / 발송가능수량 : {limitCount}명
                  <br /><span>*선물 메시지 발송 대상자가 발송가능수량을 초과하여 발송이 불가능합니다.</span>
                </p>
              )
            }
            <div className={styles.fileInfo}>
              <p>
                {sentName} 대상 : <span className={styles.fileCount}> 총 {processedUserListCount}명 </span>
                / 중복된 회원 : {repeatedUserListCount}명 제외
                / 탈퇴한 회원 : {unExistUsersListCount}명 제외
              </p>
            </div>
            ...
            {processedUserList && processedUserList.length > 0 && (
              <UserListTable title={sentName + "될 회원"} userList={processedUserList} />
            )}
          </div>
          )}
      </div>
...
export function SentContent({ title, limitCount, maxLength, placeholder, defaultValue, inputInfo, ...props }) {
  const [textValue, setTextValue] = useState(defaultValue ? defaultValue.length : 0);
  ...
  return (
    <div className={styles.textAreaBox}>
      {title && <h3>{title} {inputInfo ? <span className={styles.noticeSpan}>{inputInfo}</span> : ""}</h3>}
      <textarea
        ...
        {...props}
      ></textarea>
      <span className={styles.textAreaCount}>{`( ${textValue} / ${count}자 )`}</span>
    </div>
  )
}

export function SentInput({ title, styletype, className, inputInfo, ...props }) {
...
export function EditDataCheckBox(props) {
  ...
  const itemValue = (data, item) => {
    ...
    const result = typeof value === "string" || typeof value === "number" || typeof value === "boolean"
      ...
      : value === "" || value === null || value === undefined
        ? "없음"
        : typeof value === "object" && value !== null && value.length > 0 && item !== "targetCompany" // 브랜드는 제외
          ? ( // 거리이거나 시간대 노출인 경우
            value.map((valueDetail, index) => {
              const type = valueDetail.type === "DISTANCE" ? "거리" : "시간대"
              const text1 = valueDetail.type === "DISTANCE" ? "km 이상" : "시부터"
              const text2 = valueDetail.type === "DISTANCE" ? "km 이하" : "시까지"
              return (
                <span key={index}>{type} {valueDetail.value1 + text1} {valueDetail.value2 && valueDetail.value2 + text2}</span>
              )
            })
          )
          : item === "targetCompany"
  ...
  const ModifySave = async (type) => {
    ...
    if (type !== "advertising") {
      ...
      submitDataSet.image === null || submitDataSet.image === "" && (submitDataSet.image = "없음")
      ...
      // 광고 수정인 경우
      const advertisementsRes = await Apis.put(`/api/advertisements/${id}`, submitData)
      console.log(`광고 수정 api`, advertisementsRes)
      if (advertisementsRes.status === 200 && advertisementsRes.data.status === "success") {
        alert("수정이 완료되었습니다.")
        router.push(`/advertising`)
      } else {
        alert(`수정에 실패하였습니다. 사유 : ${advertisementsRes.data.message}`)
      }
    }
  }
  ...
  return (
    <>
      <div className={styles.jsonDataWarp}>
        <div className={styles.jsonTitle}><span>항목</span><span>원본</span><span>수정</span></div>
        <ul className={styles.jsonItemList}>
          {Object.keys(originalData).map((item, index) => {
            ...
            return (
              <li key={index} className={styles.dataCompare + (changeValue[item] ? " " + styles.change : "")}>
                <div className={styles.title}>{krChange[item] || item}</div>
                <div>
                  {item === "image" && originalData[item].includes("http")
                    ? <Image src={originalData[item]} width="50" height="50" alt="" />
                    : <span>{originalValue}</span>
                  }
                </div>
                <div>
                  {item === "image" && submitData[item].includes("http")
                    ? submitData[item] ? <Image src={submitData[item]} width="50" height="50" alt="" /> : <span>없음</span>
                    : <span>{newValue}</span>
                  }
                  {changeValue[item] ? <div className={styles.infoBox}>수정</div> : ""}
                </div>
              </li>
            )
          })}
      ...
      <div className={styles.checkModify}>
        <p>
          수정된 내용이 맞는지 확인 후 <br />
          <b>수정 완료를 눌러야만 최종적으로<br />수정이 완료됩니다.</b>
        </p>
        <Button variantStyle="color" sizeStyle="sm" onClick={() => ModifySave(type)}>수정 완료</Button>
      </div>
    </>
  )
}