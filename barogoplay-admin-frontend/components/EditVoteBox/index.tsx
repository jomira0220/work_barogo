...
import { EditVoteBoxProps } from "@/interfaces";

export default function EditVoteBox(props: EditVoteBoxProps) {
  ...
  const voteBoxClick = () => {
    ...
    const voteTitle = document.querySelector(
      `.${styles.voteTitle}`
    ) as HTMLInputElement;
    if (voteBoxOnoff) {
    ...
    <>
      <label htmlFor={styles.vote} className={styles.voteCheckbox}>
        <span>
          <input
            id={styles.vote}
            name="newVoteCheckBox"
            type="checkbox"
            onClick={() => voteBoxClick()}
          />{" "}
          신규 투표 생성
        </span>
        {/* 투표 삭제 취소 버튼 */}
