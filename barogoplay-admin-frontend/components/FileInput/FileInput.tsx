...
export default function FileInput({
  ...
  const FileInputHandler = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.currentTarget.files) return alert("파일을 선택해주세요.");

    const ImageFormData = new FormData();
    let File = e.currentTarget.files[0] as File;
    ...
    setUploadImgUrl(
      `${process.env.NEXT_PUBLIC_IMG_API_KEY}/api/images/${imgName}`
    );
      ...
      <div className={styles.fileInputWrap}>
        ...
        {fileSrc && (
          <>
            ...
            <button
              className={styles.fileImageRemove}
              onClick={() => FileInputReset()}
            >
              삭제하기
            </button>
          </>
