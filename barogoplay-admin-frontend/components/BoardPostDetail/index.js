...
import { HeartIcon, CommentIcon, VoteIcon, LockIcon, ArrowBottomRightIcon } from "@/components/Icon/Icon";
...
export default function BoardPostDetail(props) {
  ...
  const [reportStatusPop, setReportStatusPop] = useState(false) // 신고 상태 변경 관련
  ...
  useEffect(() => {
    ...
    const getUserStatus = async () => {
      const useridAll = commentData ? commentData.content.map(item => item.author) : []
  ...
  const boardString = { event: "이벤트", benefit: "제휴혜택", news: "뉴스/공지" }
  const boardKoName = {
    ...
    eventComment: boardString[router.query.boardName] + " 게시판 댓글",
  }

  return (
    <div className='basicBox'>
      <h2>{boardKoName[router.query.detailType]} 상세</h2>
        ...
        <div className={styles.boardDetailWrap}>
          <div className={styles.detailHeader}>
            ...
            {router.query.detailType === "unprocessedReport" && (
              <Button className="listCheckBtn" variantStyle="border" sizeStyle="sm" onClick={(e) => HiddenPost(e, setReportStatusPop)} data-checklist={router.query.reportId}>신고 상태 변경</Button>
            )}
          </div>
          ...
          {hashTagData && hashTagData.length > 0 && hashTagData[0] !== "" && (
            <ul className={styles.boardTag}>
              {hashTagData.map((item, index) => {
                ...
                const link = detailBoardName === "event"
                  ? `/board/event/post?boardCode=${detailBoardName}&startDate=2024-04-11&endDate=${daySet(new Date(), 0)}&searchType=hashtag&searchKeyword=${itemSet}`
                  : `/board/community/post?boardCode=${detailBoardName}&startDate=2024-04-11&endDate=${daySet(new Date(), 0)}&searchType=hashtag&searchKeyword=${itemSet}`
                return (
                  <li key={index}><Link href={link}>#{itemSet}</Link></li>
                )
              })}
            </ul>
          )}
                      ...
                      return (
                        <li
                          ...
                          className={
                            ...
                            + (router.query.commentId === String(item.id) ? ` ${styles.active}` : "")
                          }
                        >
                          {commentLevel === 1 && (
