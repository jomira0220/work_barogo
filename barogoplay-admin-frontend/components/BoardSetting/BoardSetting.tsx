...
import {
  ...
  CommentBtnProps,
  LikeBtnProps,
  CommentSubmitBtnProps,
  CommentEditBtnProps,
  CommentDelBtnProps,
  MyPostDeleteBtnProps,
  MyPostEditBtnProps,
} from "@/interfaces";

// !게시글 및 댓글 설정 버튼 클릭시 조건에 따른 버튼 노출
export const SettingBtn = (props: SettingBtnProps) => {
  const {
    className,
    isLogin,
    type,
  ...
  return (
    ...
    >
      <span className="blind">
        {type === "comment" ? "댓글 설정 버튼" : "게시글 설정 버튼"}
      </span>
      <SettingIcon />
    </button>
  );
...
//! 댓글 좋아요 버튼
export function CommentLikeBtn(props: CommentLikeBtnProps) {
  const {
    className,
    isLogin,
    item,
  ...
  const CommentLikeControl = async () => {
    ...
    if (isLiked === false) {
      ...
      if (
        likePlusPost.status === 200 &&
        likePlusPost.data.status === "success"
      ) {
        console.log("댓글 좋아요 성공");
      } else {
        console.log(
          "댓글 좋아요 처리에 실패하였습니다. 사유 : " +
            likePlusPost.data.message || "알 수 없음"
        );
      }
    } else {
      ...
      if (
        likeMinusPost.status === 200 &&
        likeMinusPost.data.status === "success"
      ) {
        console.log("댓글 좋아요 취소 성공");
      } else {
        console.log(
          "댓글 좋아요 취소 처리에 실패하였습니다. 사유 : " +
            likeMinusPost.data.message || "알 수 없음"
        );
      }

      // 좋아요 클릭시 데이터 업데이트
      const LikeRes = await Apis.get(
        `/api/boards/${boardName}/posts/${boardId}/comments?page=${commentPage}`
      ); //&size=${좋아요 클릭시에 사이즈값}
...
// !댓글 쓰기 버튼 클릭시
export function CommentBtn(props: CommentBtnProps) {
  const {
    className,
    isLogin,
    targetId,
...
export const LikeBtn = (props: LikeBtnProps) => {
  ...
  const LikeControl = async () => {
    if (isLogin === "false") {
      ...
      if (
        LikeControlRes.status === 200 &&
        LikeControlRes.data.status === "success"
      ) {
        newLikeDate(boardName, boardId);
      } else {
        alert(
          "좋아요 처리에 실패하였습니다. 사유 : " +
            LikeControlRes.data.message || "알 수 없음"
        );
      }
    }
  };
  ...
  return (
    <button className={className} onClick={() => LikeControl()}>
      <HeartIcon color={likeData.isLiked ? "#FF0000" : "#D0D4DC"} /> 좋아요{" "}
      {likeData.likeCount.toLocaleString("ko-kr")}
    </button>
  );
};

// !댓글 등록 버튼 클릭시
export const CommentSubmitBtn = (props: CommentSubmitBtnProps) => {
  const {
    className,
    commentPage,
    TargetCommentId,
  ...
  const CommentSubmit = async (TargetCommentId: null | number) => {
    ...
    if (CommentSettingOpen.content === "") {
      ...
      if (commentContent === "") {
        ...
        if (
          ...
          console.log(
            "댓글 등록 실패 사유 : ",
            commentPost.data.message || "알 수 없음"
          );
        }
      }
    } else {
  ...
  return (
    <button
      ...
      onClick={() => CommentSubmit(TargetCommentId)}
    >
      등록
    </button>
  );
...
// !댓글 수정
export const CommentEditBtn = (props: CommentEditBtnProps) => {
  const { targetId, commentData, SetCommentSettingOpen } = props;

  const CommentEdit = (targetId: number) => {
    // 수정 전 원본 댓글 내용
    const beforeComment = commentData.content.filter((item: any) => {
...
// ! 댓글 삭제
export const CommentDelBtn = (props: CommentDelBtnProps) => {
  const {
    targetId,
    detailBoardName,
    detailDataId,
  ...
  const CommentDel = async () => {
    ...
    if (
      commentDelRes.status === 200 &&
      commentDelRes.data.status === "success"
    ) {
      console.log("댓글 삭제 성공");
    } else {
      alert(
        "댓글 삭제에 실패하였습니다. 사유 : " + commentDelRes.data.message ||
          "알 수 없음"
      );
    }
  ...
  return (
    <button onClick={() => CommentDel()}>
      <RemoveArrowIcon /> 댓글 삭제하기
    </button>
  );
};

// !게시글 삭제
export const MyPostDeleteBtn = (props: MyPostDeleteBtnProps) => {
  ...
  const PostDelete = async (
    ...
    if (isHot) {
      ...
      if (postDel.status === 200 && postDel.data.status === "success") {
        alert("게시글이 삭제되었습니다.");
        router.pathname.includes("/detail/event/")
          ? router.push(`/event/${detailBoardName}`)
          : router.push(`/board/${detailBoardName}`);
      } else {
        alert(
          "게시글 삭제에 실패하였습니다. 사유 : " + postDel.data.message ||
            "알 수 없음"
        );
      }
    }
...
// !게시글 수정
export const MyPostEditBtn = (props: MyPostEditBtnProps) => {
  const { detailBoardName, targetId, router, isHot } = props;
  const postEdit = (
    detailBoardName: string,
    targetId: number,
...
export const DeclarationModal = (props) => {
  const { className, CloseHandler, type, boardName, boardId, commentId } =
    props;
  ...
  const SelectOption = [
    ...
    {
      value: "민감한 개인정보가 노출되어 있음",
      name: "민감한 개인정보가 노출되어 있음",
    },
    {
      value: "특정 대상에 대한 혐오 내용 있음",
      name: "특정 대상에 대한 혐오 내용 있음",
    },
    { value: "기타", name: "기타" },
  ];
  ...
  const DeclarationSubmit = async (type, boardName) => {
    // type: 게시글인지 댓글인지
    const reportText = onoff
      ? document.querySelector(`.${className} input[type="text"]`).value
      : "";
    const typeUrl =
      type === "comment"
    ...
    if (
      DeclarationRes.status === 200 &&
      DeclarationRes.data.status === "success"
    ) {
      alert("신고가 접수되었습니다.");
      CloseHandler();
    } else {
      alert(
        "신고 접수에 실패하였습니다. 사유 : " + DeclarationRes.data.message ||
          "알 수 없음"
      );
    }
  };
