...
export default function CustomerServiceDetail(props) {
  ...
  // const [delImgCheck, setDelImgCheck] = useState({ onoff: false, imgList: [] });
  ...
  const answerStatusCheck = async () => {
    ...
    console.log(answerPutRes, { questionIds: [customerServiceDetailData.id], qnaStatus: "답변완료" })
    if (answerPutRes.status === 200 && answerPutRes.data.status !== "error") {
      alert('문의 상태 변경이 완료되었습니다.')
    } else {
      alert('문의 상태 변경에 실패했습니다.' + answerPutRes.data.message)
    }
  }
  ...
  const answerSet = async (type, id, detailId) => {
    ...
    if (type === "edit") {
      ...
      console.log("수정", editAnswerPutRes)
      ...
      <div className='basicBox'>
        ...
        <div className={styles.detailBox}>
          <div className={styles.headerBox}>
            <div className={styles.status + (customerServiceDetailData.postMeta.qnaStatus === "답변완료" ? " " + styles.ing : "")}>{customerServiceDetailData.postMeta.qnaStatus}</div>
          ...
          {/* <div className={styles.qnaContent} dangerouslySetInnerHTML={{ __html: customerServiceDetailData.content }}></div> */}
