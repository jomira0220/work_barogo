...
import { getCustomImageBlot, keyDownEvent, uploadImgSet, getFileName, getImgSize, createSrcSet } from '@/utils/reactQuillControl';
...
export default function ReactQuillContainer({ content, className, readOnly, ...props }) {
  ...
  // !게시글 작성 및 수정 페이지에서 스크롤시 ql-toolbar가 화면에서 사라지면 고정하여 노출
  useEffect(() => {
    if (readOnly) return; // readOnly 상태에서는 실행하지 않음
    ...
    const handleScroll = () => {
      if (window.scrollY > editorTop + toolbarHeight) {
        toolbar.classList.add('ql-toolbar-fixed');
        editor.style.paddingTop = `${toolbarHeight}px`;
      } else {
        toolbar.classList.contains('ql-toolbar-fixed') && toolbar.classList.remove('ql-toolbar-fixed');
  ...
  return (
    <QuillStyleBox type={props.type} readOnly={readOnly}>
      <ReactQuill
...
const QuillStyleBox = styled.div`
  ...
  .ql-container { 
    min-height: ${(props) => props.type === "comment" ? "auto" : "45vh"};
    border: ${(props) => props.type === "comment" || props.readOnly ? "none" : "1px solid #ccc;"};
    border-radius: 0 0 5px 5px; 
    font-family: var(--font-family-1); 
    position: relative;
  }

  .ql-toolbar.ql-toolbar-fixed {
    position: fixed;
    top: 49px;
    left: 50%;
  ...
  .ql-editor {
    padding: ${(props) => props.type === "comment" ? "0" : "12px 15px"};
    height: 100%;
    position:relative;
    overflow:auto;
